...
import ShortUrlForm from "@/components/dashboard/short-url";
    ...
    <main className="px-6 md:px-20 lg:px-44 py-10 grid gap-7">
      <div className="border rounded-lg grid grid-cols-2 md:grid-cols-4 bg-card shadow-sm">
        <div className="text-center p-4 border-r border-border">
          <h2 className="text-2xl">
            <CountUp end={totalLinks.length} start={0} />
          </h2>
          <span className="text-sm text-muted-foreground">Links Created</span>
        </div>
        <div className="text-center p-4 border-r border-border">
          <h2 className="text-2xl">
            <CountUp end={totalClicks} start={0} />
          </h2>
          <span className="text-sm text-muted-foreground">Total Clicks</span>
        </div>
        <div className="text-center p-4 border-r border-border">
          <h2 className="text-2xl">
            <CountUp end={uniqueVisitors.unique} start={0} />
          </h2>
        ...
        <div className="text-center p-4">
          <h2 className="text-2xl">
            {uniqueVisitors.total > 0
              ? ((uniqueVisitors.unique / uniqueVisitors.total) * 100).toFixed(
                  1,
                )
              : 0}
            %
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Daily click trends for all your links
          </CardDescription>
        </CardHeader>
        <CardContent>
          {analyticsLoading ? (
      ...
      <Card>
        ...
        <CardContent>
          {analyticsLoading ? (
            ...
            <ChartContainer config={{ clicks: { label: "Clicks" } }}>
              <BarChart barSize={20} data={geographicData.countries}>
                <CartesianGrid strokeDasharray="3 3" />
        ...
        <Card>
          ...
          <CardContent>
            {analyticsLoading ? (
              ...
              <ChartContainer config={{ clicks: { label: "Clicks" } }}>
                <BarChart barSize={20} data={deviceData.browsers}>
                  <CartesianGrid strokeDasharray="3 3" />
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Clicks by hour of day across all links (last 30 days)
          </CardDescription>
        </CardHeader>
        <CardContent>
          {analyticsLoading ? (
        ...
        <Card>
          ...
          <CardContent>
            {analyticsLoading ? (
              ...
              <ChartContainer config={{ clicks: { label: "Clicks" } }}>
                <BarChart barSize={20} data={peakHours.peakHours}>
                  <CartesianGrid strokeDasharray="3 3" />
                  <XAxis
                    dataKey="hour"
        ...
        <Card>
          ...
          <CardContent>
            {analyticsLoading ? (
              ...
              <ChartContainer config={{ clicks: { label: "Clicks" } }}>
                <BarChart barSize={20} data={peakHours.peakDays}>
                  <CartesianGrid strokeDasharray="3 3" />
      ...
      <div className="grid gap-7 md:gap-3 md:grid-cols-2">
        <div className="border h-fit border-border rounded-lg px-4 py-4 bg-card shadow-sm">
          <div>
            <h2 className="text-lg">
              Top {top5Links.filter((link) => link.clicks).length} Links
            </h2>
            <p className="text-sm text-muted-foreground">
              the top {top5Links.filter((link) => link.clicks).length} links by
              clicks
            </p>
          </div>
            ...
            {top5Links
              ...
              .map((link) => (
                <div key={link._id} className="grid gap-2 p-4 linkList">
                  <Link
                    target="_blank"
                    className="text-sm opacity-85 flex items-center justify-between"
        ...
        <div className="border h-fit border-border rounded-lg px-4 py-4 bg-card shadow-sm">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              all your links and their clicks
            </p>
          </div>
          <div className="border-dashed rounded-lg mt-4 border border-border">
            {loading && (
